...
| `src/podcast_reader/engine/jobs.py` | Persistent job journal, FIFO single-worker execution; publishes into the shared EventBus; jobs may carry rerun `overrides` (per-job model picks; migration-safe journal load) |
...
| `app/src/renderer/` | Vanilla-TS views (Library/Reader/New/Settings + first-run Setup wizard, whose optional "AI model" section reuses the Settings chapter-provider/key flow via `chapter-onboarding.ts` — provider→docs-URL map, custom-URL toggle, putKey/putSettings save plan — and never gates Finish/Skip) + hash router + jobs/packs stores; the New view shows the full source, links a finished job to its transcript, and offers "Rerun with a different model" (a dialog — opt-in re-transcribe and/or regenerate-chapters sections — via `rerun-plan.ts`); a header theme toggle (System/Light/Dark, persisted, `app-theme.ts` + pre-paint inline script in index.html); Reader hosts the inline `media-player.ts` (docked in a left column beside the transcript — hideable via the header ✕, restored by a "Show video" control — stacking on narrow windows) (video/audio via `app://media`; YouTube via an iframe loading the engine's loopback `/v1/embed/{id}` page, `embed-protocol.ts` postMessage sync, with a "Watch on YouTube" browser fallback on embed error) wired to the transcript iframe by `sync-bridge.ts` (`pr-sync`, dual source+channel filter) |
