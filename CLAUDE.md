...
| `src/podcast_reader/html.py` | Styled canonical HTML renderer with chapters/speakers, media sync, private literal find-within-transcript, and whole/current-chapter plain-text or Markdown copy (timestamps opt-in) |
...
| `app/src/renderer/` | Vanilla-TS views (Library/Reader/New/Settings, including named OpenAI-compatible provider CRUD reusing the write-only key flow, + trust-first Setup wizard whose optional "Chapter summaries" section reuses the Settings chapter-provider/key flow via `chapter-onboarding.ts` and never gates Finish/Skip) + hash router + jobs/packs stores; flat editorial empty/progress states, quiet normal engine readiness, and implementation-leaking chapter warnings translated with technical detail retained on demand; the New view's job card leads with the video title (the link to the transcript once done), the source URL beneath, a 2-column step/model table, and a "Rerun with a different model" link; synchronized labelled Theme selectors in the header and Settings (intentional Light default, persisted Light/Dark/System, explicit System follows the OS; `app-theme.ts` + pre-paint inline script in index.html); Reader hosts the inline `media-player.ts` (docked in a left column beside the transcript — hideable via the header ✕ or a permanent toolbar Show/Hide toggle, persisted — stacking on narrow windows) (video/audio via `app://media`; YouTube via an iframe loading the engine's loopback `/v1/embed/{id}` page, `embed-protocol.ts` postMessage sync, with a "Watch on YouTube" browser fallback on embed error) wired to the transcript iframe by `sync-bridge.ts` (`pr-sync`, dual source+channel filter) |
| `app/src/renderer/src/style.css` | Editorial / Reader design system: token-driven (`--serif`/type scale/`--space-*`/`--radius-*`/`--shadow-sm`), warm-paper light + calm dark palettes, locally bundled and release-pinned Source Serif 4 display titles over Source Sans 3 body (OFL-1.1; system fail-safe stacks), list-led Library, one warm red-brown `--accent` (light `#9a3b2e` / dark `#e0876f`, matching the icon), AA contrast, reduced-motion guarded. App-shell restyle only — never touches the sandboxed `html.py` artifact or its CSP pins |
...
6. `html.py` → `<stem>.html` (styled transcript with TOC, key points, pull quotes, speaker attribution when present; per-passage `data-start`/`data-end`, inert-standalone media sync, private literal find-within-transcript, and whole/current-chapter plain-text or Markdown copy available in standalone artifacts, desktop Reader, and private-web Reader)
