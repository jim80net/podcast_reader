...
| `app/tests/install/walkthrough.mjs` | Installed-app proof driven by `test-installer.yml`: launches the NSIS-installed exe (real frozen engine, nothing mocked), asserts the engine handshake via `getEngineStatus`, walks first-run wizard → keyless YouTube captions job → Reader, uploading screenshots as run artifacts; silent uninstall closes the loop |
