...
- [x] 1.1 `app/src/main/respawn-policy.ts`: `markReady(now)`, `recordFailure(now)` →
...
- [x] 1.2 `respawn-policy.test.ts`: the full boundary table, healthy reset (≥60s resets,
  <60s does not), `reset()` — injected clock + jitter, deterministic.

## 2. Status type + IPC

- [x] 2.1 `app/src/shared/ipc.ts`: add `{ state: 'restarting'; attempt: number; maxAttempts: number }`
  to `EngineStatus`; add the `engine:restart` invoke channel to the IPC contract.
- [x] 2.2 `app/src/preload/index.ts`: expose `window.api.engineRestart()` (typed); `ipc.ts`
  handler → `manager.restart()`. Update the isolation key-set assertion if `window.api` grew.

## 3. EngineManager

- [x] 3.1 Extract the post-spawn block of `start()` into `private wireUp(handle)` (observe
...
- [x] 3.2 `handleUnexpectedExit(handle)`: guard `this.handle===handle && !quitting`; null
...
- [x] 3.3 Quit-safety: `quit()` sets `quitting=true` first; respawn re-checks `quitting`
  ...
  no surviving engine.
- [x] 3.4 `restart()`: re-entrancy guard; clear `quitting`; `RespawnPolicy.reset()`; abort
...
- [x] 4.1 `app/src/renderer/src/main.ts` `renderEngineStatus`: add the `restarting` arm
...
- [x] 5.1 Mock engine: a real `process.exit` death seam (e.g. `/__mock/crash`) so the
  spawned child genuinely exits and `childExited` resolves.
- [x] 5.2 Playwright: trigger the crash; assert the app shows `restarting` then `ready` and
  the library still loads after respawn.

## 6. Docs

- [x] 6.1 `app/README.md` (engine supervision section) + `CLAUDE.md` (engine-manager row):
