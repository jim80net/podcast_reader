...
- **Pack management in the engine**: a built-in pack registry with versioned manifests, compat ranges, and an unpublished/`unavailable` state for entries without a published artifact (per S5); `GET /v1/packs` (statuses + detected hardware + recommendations), `POST /v1/packs/{id}/install` (checksummed, resumable with sha256-identified staging partials so pin bumps restart cleanly (per S2), atomic, manifest-written-last), `DELETE /v1/packs/{id}` (manifest deleted first — atomic not-installed, safe against running jobs, per S1); startup validation flags incompatible packs after an app update moves the compat range and checks manifest-listed files for existence + size (per S8). Pack downloads run **through the engine** with progress on the existing `GET /v1/events` SSE stream (rationale in design; pack events carry no `job_id`, per Q5) — on a dedicated installer thread, never the job queue.
...
- **Diarization worker pack** (spike verdict: GO): a separately frozen pyannote+torch CPU worker plus pre-seeded HF pipeline cache, hosted on GitHub Releases; engine pre-converts audio to 16 kHz mono WAV with its managed ffmpeg, the worker emits `turns.json`, and the engine performs the max-overlap speaker merge (pure stdlib) before render. `html.py` learns to render speaker labels. The parent design's cut-line still applies: if the worker freeze proves non-viable during implementation, the diarization task group detaches **entirely** — no dormant merge/setting code lands, and the registry entry stays unpublished (per S5) — without blocking the rest.
- **Tools seeding + yt-dlp self-update**: bundled yt-dlp/ffmpeg/ffprobe seeds are reconciled into `<data_dir>/tools/` at engine startup (newer-wins via a versions manifest, completing the tool-resolution spec's seeding-time contract); `yt-dlp -U` runs against the user-data copy on a throttled schedule and on structured extraction failure (`download_failed`, per S7) with one retry — the retry hook lives in `ytdlp.py`, gated purely on the resolved binary residing in the user-data tools dir, no engine/CLI flag (per Q3) — never against the install dir.
- **App setup UI**: a first-run setup wizard (hardware-based recommendations, sets `whisper_device` from detected hardware (per S4), live progress, resumable, skippable, re-runnable from Settings) and a Settings pack-management section (states, sizes, install/uninstall, incompatible/failed→re-download, license attributions, cuda-without-pack advisory (per S4/Q2)).
- **Packaging & CI**: `packaging/` productionizes the spike's engine.spec (real engine + whisper worker + tool seeds + custom hooks) with a build script feeding `app/scripts/dist.mjs --engine-dir`; the CI `frozen-smoke` job is upgraded from the spike stub to building the **real** engine, completing the authed discovery handshake, asserting the frozen engine reports the pyproject version (per S3), installing the tiny model pack through the API, and transcribing a 5-second fixture end-to-end on both matrix legs (per Q1).
