...
The extension SHALL submit the active tab's URL as an engine job from the toolbar action (URL obtained via the click-granted `activeTab`) and from a context-menu item on any page (URL from `info.pageUrl`), using `POST /v1/jobs` with `requires_confirmation: false`: a deliberate click inside the user-installed, token-authed extension carries user intent exactly as the app's New view does, while the confirmation gate remains scoped to the unauthenticated `podcast-reader://` channel (per the parent design's F10). Submitted job ids SHALL be recorded in `chrome.storage.local` as a bounded most-recent-first list.

#### Scenario: Toolbar click submits directly
- **WHEN** the user clicks the toolbar action on a page while paired and the engine is reachable
- **THEN** a job for the tab URL is created in `queued` (no `awaiting-confirmation` stop) and its id is tracked
