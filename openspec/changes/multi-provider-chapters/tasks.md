...
- [ ] 1.1 `src/podcast_reader/providers.py`: `ProviderSpec` TypedDict (`base_url, default_model, key_env, max_tokens`), `PROVIDERS` registry per the design table (verified base URLs/models; deepseek `deepseek-v4-flash` @ 8192, others 16384), custom-URL validation (https or localhost http); unit tests incl. validation scenarios
