...
- [ ] 1.5 `engine/app.py`: `GET /v1/providers` — registry ids + default model + key-available boolean (pushed-or-env, boolean only); response never contains key material; TestClient tests: list includes exactly the six registry ids, response contains no key material with keys pushed and env vars set (per P4)
...
- [ ] 7.3 Real-engine spawn smoke (integration-marked): launch app with dev fallback against `uv run podcast-reader serve`; assert sentinel → discovery file → authed health → Library renders; additionally assert exact key-set equality of real `JobRecord`/`LibraryEntry`/`EngineSettings` payloads vs the `src/shared/types.ts` mirrors (per Q4); proves the TS handshake mirror matches `engine/process.py`
